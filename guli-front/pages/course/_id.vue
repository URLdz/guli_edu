                    ...
                    <div class="lh-menu-wrap">
                      <menu id="lh-menu" class="lh-menu mt10 mr10">
                        <ul>
                          ...
                          <li
                            class="lh-menu-stair"
                            v-for="chapter in chapterVedioList"
                            :key="chapter.id"
                          >
                            <a
                            ...
                            <ol class="lh-menu-ol" style="display: block">
                              <li
                                ...
                                :key="video.id"
                              >
                                <a v-if="Number(courseWebVo.price) === 0 || isBuy"
...
export default {
  ...
  methods: {
    //=========================订单============================
    createOrders() {
      orderApi.createOrders(this.comment.courseId).then((response) => {
          console.log(response.data);
          if (response.data.code == 20000) {
            this.$router.push({
              path: "/orders/" + response.data.data.orderNo,
            });
          } else {
    ...
    initComment() {
      console.log();
      comment
        .getPageList(this.current, this.limit, this.comment.courseId)
        .then((response) => {
    ...
    gotoPage(current) {
      comment
        .getPageList(current, this.limit, this.courseId)
        .then((response) => {
          this.data = response.data.data;
        });
    },
    //=========================课程============================
    initCourseInfo() {
      courseApi.getCourseInfo(this.comment.courseId).then((response) => {
        (this.courseWebVo = response.data.data.courseWebVo),
        (this.isBuy = response.data.data.isBuy),
        (this.chapterVedioList = response.data.data.chapterVedioList);
      });
    },
  },
  // asyncData({ params, error }) {
  ...
  //       chapterVedioList: response.data.data.chapterVedioList,
  //     };
  //   });
  // },
};
